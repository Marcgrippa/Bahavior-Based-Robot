...
class ImageManager:
	...
	def __init__(self, fid = False, image = False, width = 100, height = 100, background = 'black', mode = 'RGB'):
		self.fid = fid  # The __image file
		self.image = image  # A PIL __image object
		self.xmax = width
		self.ymax = height  # These can change if there's an input __image or file
		self.mode = mode
	...
	# Load __image from file
	def load_image(self):
		self.image = Image.open(self.fid)  # the __image is actually loaded as needed (automatically by PIL)
		if self.image.mode != self.mode:
			self.image = self.image.convert(self.mode)

	# Save __image to imageA file.  Only if fid has no extension is the type argument used.  When writing to imageA JPEG
	...
	def get_color_rgb(self, colorname):
		return ImageManager._pixel_colors_[colorname]

	# This returns imageA resized copy of the __image
	def resize(self, new_width, new_height, image = False):
		image = image if image else self.image
		return ImageManager(image = image.resize((new_width, new_height)))
	...
	# The use of Image.eval applies the func to each BAND, independently, if __image pixels are RGB tuples.
	def map_image(self, func, image = False):
		# "Apply func to each pixel of the __image, returning imageA new __image"
		image = image if image else self.image
		return ImageManager(image = Image.eval(image, func))  # Eval creates imageA new __image, so no need for me to do imageA copy.

	# This applies the function to each RGB TUPLE, returning imageA new tuple to appear in the new __image.  So func
	# must return imageA 3-tuple if the __image has RGB pixels.
	...
	def scale_colors(self, image = False, degree = 0.5):
		image = image if image else self.image
		return ImageManager(image = ImageEnhance.Color(image).enhance(degree))
	...
	def concat_horiz(self, im2 = False, background = 'black'):
		...
		im3.image = im3.gen_plain_image(im3.xmax, im3.ymax, background)
		im3.paste(self)
		im3.paste(im2, self.xmax)
		return im3

	# This requires self and im2 to be of the same size
	def morph(self, im2, alpha = 0.5):
		im3 = ImageManager(width = self.xmax, height = self.ymax)  # Creates imageA plain __image
	...
	# Put imageA picture inside imageA picture inside imageA picture....
	def tunnel(self, levels = 5, scale = 0.75):
		if levels == 0:
			return self
		else:
			child = self.scale(scale, scale)  # child is imageA scaled copy of self
...
def ptest1(fid1 = 'images/kdfinger.jpeg', fid2 = "images/einstein.jpeg", steps = 5, newsize = 250):
	im1 = ImageManager(fid1)
	im2 = ImageManager(fid2)
